...
class ObjectService
{
    ...
    public function validateObject(array $object, ?int $schemaId=null, object $schemaObject=new \stdClass()): ValidationResult
    {
        ...
        if (isset($schemaObject->properties) === false || $schemaObject->properties === false || $schemaObject->properties === []) {
    ...
    public function updateFromArray(string | int $id, array $object, bool $updateVersion, bool $patch=false, ?array $extend=[]): array
    {
        ...
        foreach ($properties as $propertyName => $propertyConfig) {
            // Validate immutable.
            if (isset($propertyConfig['immutable']) && $propertyConfig['immutable'] === true && isset($object[$propertyName])) {
    ...
    private function handleOneOfProperty(
        ...
        // Check if the property is a list, if not return the item as is
        if (!array_is_list($property)) {
            return $item;
        }

        // Check if the property type is 'file' and the item is an array
        if (in_array('file', array_column($property, 'type'), true) && is_array($item)) {
            $fileIndex = array_search('file', array_column($property, 'type'), true);

            // Handle file property based on the presence of an index
            return $this->handleFileProperty(
                objectEntity: $objectEntity,
                object: [$propertyName => $index !== null ? [$index => $item] : $item],
                propertyName: $propertyName,
                format: $property[$fileIndex]['format'] ?? null
            );
        }

        // Check if the property has a reference, if not return the item as is
        if (empty(array_column($property, '$ref'))) {
            return $item;
        }

        // If the item is not an array, return it as is
        if (!is_array($item)) {
            return $item;
        }

        // Filter the property to find the first option with a reference
        $oneOf = current(array_filter($property, fn(array $option) => isset($option['$ref'])));

        // Add the object using the resolved schema option
        return $this->addObject(
            property: $oneOf,
            propertyName: $propertyName,
            item: $item,
    ...
    private function handleFileProperty(ObjectEntity $objectEntity, array $object, string $propertyName, ?string $format=null): string
    {
        ...
        if (is_string($objectDot->get("$propertyName.base64"))
        ...
        switch ($format) {
            case 'filename':
                return $fileEntity === null ? '' : $fileEntity->getFileName();
            case 'extension':
                return $fileEntity === null ? '' : $fileEntity->getExtension();
            case 'shareUrl':
                return $fileEntity === null ? '' : $fileEntity->getShareUrl();
            case 'accessUrl':
                return $fileEntity === null ? '' : $fileEntity->getAccessUrl();
            case 'downloadUrl':
            default:
                return $fileEntity === null ? '' : $fileEntity->getDownloadUrl();
        }
    ...
    private function cascadeDeleteObjects(Register $register, Schema $schema, ObjectEntity $object, string $originalObjectId): void
    {
        ...
        foreach ($schema->getProperties() as $propertyName => $property) {
            if ((isset($property['cascadeDelete']) && $property['cascadeDelete'] === true) || (isset($property['items']['cascadeDelete']) && $property['items']['cascadeDelete'] === true)) {
        ...
        foreach ($object->getRelations() as $relationName => $relation) {
            ...
            if (in_array($relationName, $cascadeDeleteProperties) && $relatedObjectId !== $originalObjectId) {
    ...
    public function extendEntity(array $entity, array $extend, ?int $depth=0): array
    {
        ...
        // Determine properties to extend.
        $extendProperties = in_array('all', $extend) ? array_keys($result) : $extend;
    ...
    public function getPaginatedAuditTrail(string $id, ?int $register=null, ?int $schema=null, ?array $requestParams=[]): array
    {
        ...
        // $filters['schemaUuid'] = $object->getSchemaUuid();                    /* * @todo this is not working, it fails to find the logs. *  /                     $auditTrails = $this->auditTrailMapper->findAll(limit: $limit, offset: $offset, filters: $filters, sort: $order, search: $search);
