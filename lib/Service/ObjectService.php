...
class ObjectService
{
    ...
    ): string|array
    {
        ...
        if ($itemIsID === true) {
            $nestedObject = $this->getMapper('objectEntity')->find($item);
        } else {
...
//			$objects = array_map(fn($obj) => $obj->jsonSerialize(), $objects);
        ...
        if (!empty($extend)) {
            ...
            foreach ($extend as $property) {
                ...
                $extendedObjects = array_filter($relatedObjects, function($obj) use ($value) {
                    if(is_array($value)){
                ...
                if($schema->getProperties()[$property]['type'] !== 'array') {
                    ...
                    if (strpos($property, '.') !== false) {
                        ...
                        $extendedObjects = $this->renderEntity(
                            $extendedObjects,
                            $extend,
                            $depth + 1,
                            $filter,
                        ...
                        $dotEntity->set($mainProperty, $extendedObjects);
                    } else {
                        // Set the related object directly in the dotEntity for non-nested properties
                        $dotEntity->set($property, $extendedObjects);
                    }
                } else {
                    $dotEntity->set($property, array_values(array_map(function(ObjectEntity $object){return $object->jsonSerialize();},$extendedObjects)));
                }
